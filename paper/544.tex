...
\titlerunning{Federated Learning for Massive MIMO Channel Estimation}

\author{Jagadeesh Kakarla\inst{1} \and Koppaku Krishna Chaitanya\inst{1} \and Kappala Akshay\inst{1} \and Dammala Basava Sri ram\inst{1} \and Yaparla Sainatha reddy\inst{1}}

\authorrunning{J.~Kakarla et al.}

\institute{Indian Institute of Information Technology, Design and Manufacturing, Kancheepuram, India\\
...
(ResUNet) architectures via FedAvg and FedProx on IID and
Non-IID clients, respectively. Our reproducibly available benchmark confirms that our federated approach provides good estimation results and, in particular, outperforms the centralized counterpart when FedAvg is run in IID setting by only 0.32 dB (-20.04 dB vs. -19.62 dB NMSE). In addition, we analyze the effect of Non-IID data on channel estimation and show that there is up to a 0.3 dB accuracy gap that can be mitigated via FedProx.
...
architecture of Massive MIMO system is proposed. With many antennas on the base station (BS) side, Massive MIMO uses spatial multiplexing and directional beamforming to support multiple users through the same frequency-time resources at the same time.
...
In this paper, we attempt to conduct an exhaustive analysis for channel estimation in FL based on both IID and Non-IID scenarios. The performances of FedAvg and the proximal regularization technique called FedProx have been compared using both lightweight CNN architecture and heavy ResUNet architecture.
...
They reported that their method results in an overhead reduction factor of approximately 16 compared to CL in their framework. For further background on machine learning and federated approaches in current wireless networks, including RIS, refer to the comprehensive survey paper by Faisal and Choi~\cite{faisal2022rismlsurvey}. Foundational papers in the field of Federated Learning are FedAvg~\cite{mcmahan2017fedavg} and FedProx~\cite{li2020fedprox}.
...
Heterogeneity \newline analysis & Non-IID challenge discussed & Explicit IID/Non-IID protocol and quantified deltas \\
Update-channel \newline robustness & Noisy, quantized, partial-loss model updates & Communication/runtime analysis; update-channel robustness left for future extension \\
...
\caption{Architecture diagram of the Massive MIMO Federated Learning system.} 
\label{fig:system_model}
\end{figure}

Fig.~\ref{fig:system_model} illustrates the end-to-end federated pipeline used in the proposed work. The diagram highlights the clear separation between the edge-level User Equipments (UEs) and the central Base Station (BS). Instead of transmitting raw, high-dimensional channel samples to the BS---which causes heavy communication overhead---the clients perform local CSI model updates using their private data. The server then aggregates these model parameters. This setup follows the standard privacy-preserving FL paradigm and motivates our FedAvg/FedProx formulations under heterogeneous wireless clients~\cite{mcmahan2017fedavg,li2020fedprox,kairouz2021survey}.
...
\end{equation}
where $F_k(\theta_k)$ is the local loss function of client $k$, and $\mu \ge 0$ is the proximal regularization parameter.

\section{Proposed Methodology}
This proposed methodology follows the below pipeline which consists of six stages, namely; (i) channel generation using the DeepMIMO ray tracing simulator, (ii) adding AWGN noise at varying SNR values, where power normalization is done for each sample, (iii) global normalization and dataset split for train and test dataset, (iv) client data distribution following IID and Non-IID distribution, (v) Federated Learning-based model training using FedAvg and FedProx algorithms, where two different CNN and ResUNet based architectures will be trained, and (vi) evaluation of models' performance through the NMSE metric using the holdout test dataset. Model training will be carried out locally using all clients through the Adam optimizer to minimize the MSE cost function.

\subsection{Dataset Construction}
The dataset is derived from DeepMIMO O1\_60~\cite{alkhateeb2019deepmimo}. We have selected 6000 users from BS-0, configure an 8$\times$8 BS array, 1$\times$1 UE array, and 64 OFDM subcarriers. The AWGN noise is added to the SNR of \{0,5,10,15,20\} dB after sample-wise normalization using the same random number generator seed (42). Real/imaginary channel data are saved as well.
...
\textbf{CNN (Primary Benchmark):} A lightweight residual convolutional neural network architecture that learns the residual correction to noise ($\Delta H = H_{\text{noisy}} - H_{\text{clean}}$) as opposed to learning a mapping of the entire channel. The model comprises a feature extraction head (a 2D Convolutional layer that maps from 2 to 32 channels,
...
\end{equation}
where $\mathcal{D}_k^u$ is the unique subset of client $k$, $\mathcal{D}_k^s$ is the shared global subset, and $\rho \in [0, 1]$ represents the ratio of unique samples to local dataset size (with $\rho=0.6$). In CNN FL, Non-IID client sets are generated by dedicated split scripts that induce structured per-client skew before training.
...
\end{equation}
which requires $O(d^3)$ matrix inversion for $d=8192$. This provides a benchmark upper bound on linear estimation performance under the same split/metric protocol as the deep models.

\section{Results and Structured Analysis}
\subsection{Centralized and Federated Performance Summary}
\noindent\textbf{Purpose.} Understanding the global environment of performance before moving ahead.
...
\noindent\textbf{Observation.} Test statistics of CNNs demonstrating the efficiency of the model as maximum in case of FedAvg-IID algorithm at -20.0381 dB as compared to the highest seed value of centralized approach at -19.6215 dB. The centralized tests yield an average of -19.4227 dB on the other hand, for FedAvg-IID, the average turns out to be -19.1190 dB which is higher by 0.3036 dB from the average of centralized tests.

\noindent\textbf{Interpretation.} Federated Learning with statistical aggregation efficiency can be considered as a potential approach; nevertheless, it can vary with variations in the data. Results of FedAvg-IID demonstrate that the Federated Learning method can be employed as an alternative, which, in turn, might become better than centralized learning methods upon optimization, although overall performance requires evaluation.

Table~\ref{tab:cnn_stats} provides the primary CNN benchmark summary across centralized and federated settings. The mean and standard-deviation trends confirm two key findings: (i) FedAvg-IID remains close to centralized performance on average, and (ii) distribution heterogeneity introduces a measurable degradation from IID to Non-IID, motivating robustness-oriented FL analysis. These findings are visually corroborated by Fig.~\ref{fig:f1}, which illustrates the baseline reproducibility and the narrow variance across different random seeds in the centralized setup, establishing a stable upper bound for comparison.

\noindent\textbf{Implication.} Both best and mean metrics must be considered while making any decision since the best assesses the potential whereas the mean analyzes the stability.
...
\noindent\textbf{Purpose.} The objective of the experiment is to study the effects of global rounds and epochs (number of local iterations) on the quality of estimates.

\noindent\textbf{Observation.} As illustrated in Fig.~\ref{fig:f2}, the NMSE rapidly improves during the initial global rounds before reaching a phase of diminishing returns, reflecting the typical convergence rate of FedAvg on IID data. Furthermore, Fig.~\ref{fig:f3} highlights the impact of local computations; increasing the number of local epochs generally improves the global convergence performance, though it introduces a trade-off between local fitting and potential client drift.

\noindent\textbf{Interpretation.} This represents the typical behavior of FL optimization where each iteration of communications reduces the bias significantly, while each subsequent iteration optimizes the network. Increased epoch sizes help in better fitting locally, although there might be issues like drifting in case of heterogeneous datasets.

\noindent\textbf{Implication.} Knee point selection should be done for the operating point.
...
\noindent\textbf{Purpose.} To establish trajectory information at the round level on stability and learning dynamics, rather than only end-point information.

\noindent\textbf{Observation.} This is explicitly visible in the convergence trajectories plotted in Fig.~\ref{fig:f8}. The figure tracks the validation NMSE across global rounds for both IID and Non-IID regimes. As depicted, the FedAvg-IID curve (often smoother and converging faster) maintains a consistent downward trajectory, whereas the Non-IID curves exhibit greater variance and instability during the optimization process, ultimately converging to a higher error floor.

\noindent\textbf{Interpretation.} The graph of convergence illustrates that heterogeneity is an intermediary issue and not simply an end-issue since it impacts the optimization procedure.

\noindent\textbf{Implication.} The algorithms used in Non-IID Learning should not depend solely on NMSE but instead on trajectory stability.
...
\noindent\textbf{Purpose.} Cost incurred by changing distribution of the IID distribution and understand FedProx behavior relative to FedAvg.

\noindent\textbf{Observation.} Changing distributions is noticeably more expensive in the Non-IID case. Fig.~\ref{fig:f4} visually highlights this heterogeneity penalty by contrasting the NMSE distributions of IID versus Non-IID settings, showing a clear performance gap and increased variance when client data is skewed. Specifically, comparing FedAvg-IID and FedAvg-Non-IID reveals an average difference of 0.2707 dB. There exists statistical significance between them (Welch $p=0.0289$). Comparing the performance of FedAvg-Non-IID and FedProx-Non-IID gives the average difference to be 0.2800 dB (Welch $p=0.0044$).

\noindent\textbf{Interpretation.} Effect of Non-IID data can be measured statistically. FedProx doesn’t give superior performance regarding mean NMSE, but stability and gains from specific regime are its strengths.

\noindent\textbf{Best/Mean/Worst Interpretation.} To further interpret this robustness, Fig.~\ref{fig:f7} summarizes the Best, Worst, and Mean statistics across the tested algorithms. The larger variation bands in the Non-IID regime underscore the system's increased sensitivity to random seeds and hyperparameter choices, whereas narrower bands in IID settings imply stable learning.
...
\noindent\textbf{Implication.} For obtaining robust results, variability and confidence bands have to be considered alongside the means.

\subsection{Cross-Architecture Consistency (ResUNet Add-on)}
\noindent\textbf{Purpose.} Check the sensitivity of heterogeneity findings to the model type.
...
\noindent\textbf{Observation.} Table~\ref{tab:resunet} outlines the performance metrics for the heavier ResUNet architecture across different settings. More importantly, Fig.~\ref{fig:cross} compares the performance curves of the simple CNN network with the complex ResUNet network. The plot demonstrates that despite the vast difference in model capacity, the core trend directions are perfectly ported: FedAvg-IID remains the strongest federated setup, and introducing Non-IID data causes a substantial and consistent degradation across both architectures. Specifically, FedAvg-Non-IID degrades by +1.7150 dB vs FedAvg-IID.

\noindent\textbf{Interpretation.} Heterogeneity penalty persists across shallow and encoder-decoder architectures, indicating a data-distribution effect rather than a single-model artifact.

\noindent\textbf{Implication.} Mitigation strategies for Non-IID should be treated as pipeline-level priorities, not architecture-only fixes.

\subsection{Classical Baseline and Communication Analysis}
\noindent\textbf{Purpose.} Place deep models relative to classical LS and quantify systems cost.
...
\end{equation}
where $P$ is the number of model parameters, $K$ is the total number of clients, and $R$ is the number of global communication rounds.
...
\noindent\textbf{Interpretation.} Communication overhead is linearly dependent on both the size of the model and the iterations in federated learning. The architecture that improves NMSE the most may also be the least communication-efficient.
...
\noindent\textbf{Implication.} Communication-conscious selection of models becomes vital for implementing FL practically. Finally, Table~\ref{tab:comm} encapsulates the communication cost snapshots for the CNN. It illustrates the practical trade-off between communication efficiency and model accuracy; configurations demanding more frequent communication events (e.g., R50, E5) yield the highest NMSE performance but require a significantly larger total uplink payload.
