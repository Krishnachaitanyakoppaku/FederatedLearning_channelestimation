"""
cnn_model.py — Lightweight Residual CNN for MIMO Channel Estimation

Architecture Overview:
    Input  :  (batch, 2, 64, 64)   — real & imaginary parts of noisy channel
    Output :  (batch, 2, 64, 64)   — denoised real & imaginary parts

    The network learns the residual  ΔH = H_clean − H_noisy  and adds it
    back, which is easier to learn than the full mapping (ResLearning).

    Layer summary
    ─────────────────────────────────────────────
    Conv2d(2 → 32)  + BN + ReLU       encoder / feature extraction
    ResidualBlock(32 → 32)             skip-connected refinement
    Conv2d(32 → 2)                     projection back to channel dims
    ─────────────────────────────────────────────
    Total parameters : ~28 k   (well under the 100 k budget)
