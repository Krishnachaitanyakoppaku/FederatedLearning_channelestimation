% One-column layout is intentionally used for CVIP format.
...
We include two classical references at 10 dB: LS and a calibrated scalar LMMSE baseline. LS uses direct noisy observation, i.e., $\hat H_{\mathrm{LS}}=\tilde H$. LMMSE is calibrated from train split statistics only (no test leakage):
\begin{equation}
\hat H_{\mathrm{MMSE}} = \mu_H + \alpha(\tilde H-\mu_H),\qquad
\alpha = \frac{\sigma_H^2}{\sigma_H^2+\sigma_W^2},
\end{equation}
where $\mu_H,\sigma_H^2$ are estimated from clean train labels and $\sigma_W^2$ from train residual noise $(\tilde H-H)$. This provides a calibrated classical benchmark under the same split/metric protocol as deep models.
...
\begin{table}[t]
\caption{Classical and Deep Baselines at 10 dB (Test Split)}
\label{tab:ls}
\centering
...
Method & NMSE (dB) & Note \\
\midrule
LS & -10.0022 & $\hat H=\tilde H$ \\
Calibrated MMSE & -10.4169 & Train-calibrated scalar LMMSE \\
CNN (CL mean) & -19.4227 & Improvement: +9.42 dB vs LS, +9.01 dB vs MMSE \\
ResUNet (CL mean) & -22.1210 & Improvement: +12.12 dB vs LS, +11.70 dB vs MMSE \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Observation:} calibrated MMSE improves over LS by 0.41 dB, as expected for linear Bayesian shrinkage under AWGN. Both deep models remain substantially better than LS and MMSE in this benchmark: CNN by +9.01 dB over MMSE and ResUNet by +11.70 dB over MMSE.

\textbf{Interpretation:} closing the classical-baseline gap confirms that deep models are not only outperforming a weak baseline (LS), but also a calibrated MMSE reference under the same evaluation split.

\textbf{Implication:} the FL value proposition is strengthened: federated deep models can be compared against stronger classical references while preserving privacy-aware training.
...
\subsection{FedProx Stability Diagnosis in This Benchmark}
The observed FedProx instability is consistent with a hyperparameter-regime mismatch rather than a universal algorithmic failure. Three pieces of evidence support this:
\begin{itemize}
...
\begin{itemize}
    ...
    \item Current MMSE baseline is calibrated scalar LMMSE; full covariance/pilot-aware MMSE is not yet integrated.
...
\begin{itemize}
    ...
    \item Extend MMSE to full covariance/pilot-aware formulation and report paired classical-vs-DL robustness studies.
...
\emph{IEEE Transactions on Wireless Communications}, vol. 21, no. 6, pp. 4255--4268, Jun. 2022, doi: 10.1109/TWC.2021.3128392.

\bibitem{faisal2022rismlsurvey}
K. M. Faisal and W. Choi,
``Machine learning approaches for reconfigurable intelligent surfaces: A survey,''
\emph{IEEE Access}, vol. 10, pp. 27343--27367, 2022, doi: 10.1109/ACCESS.2022.3157651.
...
The following supplementary figures are intentionally placed after references and can be retained, reordered, or removed based on final page budget.
...
\caption{Supplementary S3: Communication-efficiency scatter. Visualizes Pareto behavior between communication events and NMSE.}
\label{fig:s4}
\end{figure}
...
\caption{Supplementary S4: ResUNet add-on mean with error bars. Confirms trend consistency under architecture change.}
\label{fig:s5}
\end{figure}
...
\caption{Supplementary S5: ResUNet seed-wise distribution. Provides transparent variability view per setting.}
\label{fig:s6}
\end{figure}
...
\caption{Supplementary S6: ResUNet delta analysis (IID/Non-IID/FedProx effects).}
\label{fig:s7}
\end{figure}
...
\caption{Supplementary S7: ResUNet runtime profile. Supports systems-level resource discussion.}
\label{fig:s8}
\end{figure}
